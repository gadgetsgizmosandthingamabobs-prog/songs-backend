...
app.post('/api/song/create', async (req, res) => {
  try {
    ...
    const musicPayload = {
      ...
      gpt_description_prompt: `A custom song for ${recipient || 'someone special'} named ${name}. Occasion: ${occasion}. Memories/Details: ${memories || ' heartfelt and loving'}`
    };
...
app.get('/api/check-status', async (req, res) => {
  try {
    ...
    if (statusData.status === 'succeeded' || statusData.status === 'completed' || statusData.audio_url) {
      ...
      session.audioUrl = statusData.audio_url || statusData.url;
